...
public class TriviaGameActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void fillData(){
        ...
        data.add("\nאמא שלך חזרה הביתה מבית חולים ואמרה שהיא לא מרגישה טוב\n ורוצה ללכת לנוח. מה תעשה?");
        data.add("תשחק בחדר שלך");
        data.add("תדליק טלויזיה על ווליום גבוה");
        data.add("תשחק בחדר של אמא ותעשה רעש");
        data.add("תבקש מאמא להכין לך חביתה כי אבא תמיד שורף אותה");
        data.add("1");
        data.add("מה היית רוצה לעשות אחרי חזרה מהרופא?");
        data.add("מה הכי מפריעה לך כאתה מנסה לנוח?");
        data.add("רעש, רעש, רעש");

        data.add("\n איך נראה לך שאתה יכול לעזור להורים שלך?");
        data.add("להכין מתנות קטנות");
        data.add("להצטלם כמה שיותר");
        data.add("להגיד שאתה אוהב אותם");
        data.add("כל התשובות נכונות");
        data.add("4");
        data.add("בטוח רק זה?");
        data.add("אולי עוד תשובה מתאימה?");
        data.add("ומה עם כל השאר האפשרויות?");

        data.add("הורה שלך מתחיל לנקות את הבית,\n אבל אחרי כמה דקות הוא לוקח הפסקה לנוח.\n  מה תעהשה?");
        data.add("אלך לחדר בשביל לא להפריע");
        data.add("לוקח את המטלות על עצמי ועוזר");
        data.add("מלכלך את הבית עוד יותר, גם ככה מנקים");
        data.add("מזכיר להורה שצריך גם לנקות בחדר שלי");
        data.add("2");
        data.add("בטוח שככה תעשה?");
        data.add("אולי אפשר לעשות משהו?");
        data.add("אפשר גם לנקות קצת ולעזור");

        data.add("\nמה אפשר לעשות בבית חולים בזמן שההורה שלך מקבל טיפול?");
        data.add("לשחק בטלפון");
        data.add("לקרוא ספר");
        data.add("כל התשובות נכונות");
        data.add("לספר להורה על משהו מעניין מבית הספר");
        data.add("3");
        data.add("רק זה?");
        data.add("בטוח שאי אפשר לעשות משהו אחר?");
        data.add("אולי אפשר לעשות כמה דברים מהרשימה?");

        data.add("\nמה נראה לך שהכי ישמח את ההורה שלך לשמוע?");
        data.add("הלכי מכות בבית הספר");
        data.add("אין לי כוח לדבר איתך");
        data.add("אני אוהב אותך");
        data.add("לא רוצה לעשות שיעורים");
        data.add("3");
        data.add("לא נראה לי שזה משמח");
        data.add("זה בטוח לא היה משמח אף אחד");
        data.add("תחשוב על משהו יותר נחמד");
